...
const featureNames = [
  ...
  // Existing QB/WR season context
  'seasonPffScore',
  'hasSeasonPff',

  // QB-specific season traits
  'qbBttRate',
  'qbTwpRate',
  'qbAccuracy',
  'qbPressureToSack',
...
function buildRows() {
  ...
  for (const r of draftRows) {
    ...
    row.seasonRows = getSeasonRows(row, pffSeasonStore);
    rows.push(row);
  }
  ...
  return {
    ...
    meta: {
      ...
      qbSeasonMatches: rows.filter((r) => r.pos === 'QB' && r.seasonPffScore != null).length,
      wrSeasonMatches: rows.filter((r) => r.pos === 'WR' && r.seasonPffScore != null).length,
    },
  };
}

function getSeasonRows(player, store) {
  const files = pffSeasonFilesForPos(player.pos);
  const playerKey = clean(player.name);

  return files
    .flatMap((file) => store[file] ?? [])
    .filter((row) => {
      const name = rowName(row);
      const season = rowSeason(row);
      return clean(name) === playerKey && season != null && season < player.year;
    })
    .sort((a, b) => (rowSeason(a) ?? 0) - (rowSeason(b) ?? 0));
}

function weightedSeasonMetric(player, keys) {
  const rows = player.seasonRows ?? [];
  ...
  for (const row of rows) {
    ...
    const weight =
      player.pos === 'QB'
        ? rowNumber(row, ['dropbacks', 'attempts', 'aimed_passes', 'pass_attempts']) ?? 1
        : rowNumber(row, ['routes', 'targets', 'receptions', 'snaps']) ?? 1;

    total += value * Math.max(1, weight);
    weightTotal += Math.max(1, weight);
  }
...
function makeFeatures(player) {
  ...
  return {
    ...
    qbBttRate: weightedSeasonMetric(player, ['btt_rate', 'BTT%', 'big_time_throw_rate']) ?? 0,
    qbTwpRate: weightedSeasonMetric(player, ['twp_rate', 'TWP%', 'turnover_worthy_play_rate']) ?? 0,
    qbAccuracy: weightedSeasonMetric(player, ['accuracy_percent', 'adjusted_completion_percent', 'adj_completion_pct']) ?? 50,
    qbPressureToSack: weightedSeasonMetric(player, ['pressure_to_sack_rate', 'p2s_rate']) ?? 0,
    qbTimeToThrow: weightedSeasonMetric(player, ['avg_time_to_throw', 'time_to_throw']) ?? 2.8,

    wrYprr: weightedSeasonMetric(player, ['yprr', 'yards_per_route_run']) ?? 1.5,
    wrRouteGrade: weightedSeasonMetric(player, ['route_grade', 'grades_pass_route']) ?? 50,
    wrDropRate: weightedSeasonMetric(player, ['drop_rate']) ?? 5,
    wrAdot: weightedSeasonMetric(player, ['adot', 'avg_depth_of_target']) ?? 10,
...
function trainModel(rows, label, lambda = 4.0) {
...
function walkForward(rows, scopeName) {
  ...
  for (const testYear of years) {
    ...
    if (train.length < 80 || test.length < 8) continue;

    const { model } = trainModel(train, `${scopeName}-through-${testYear - 1}`);
...
const qbRows = rows.filter((row) => row.pos === 'QB');
const wrRows = rows.filter((row) => row.pos === 'WR');
const qbWrRows = rows.filter((row) => row.pos === 'QB' || row.pos === 'WR');

const globalAll = trainModel(rows, 'GLOBAL_WITH_QB_WR_SEASON_CONTEXT');
const qbWr = trainModel(qbWrRows, 'QB_WR_COMBINED');
const qbOnly = trainModel(qbRows, 'QB_ONLY');
const wrOnly = trainModel(wrRows, 'WR_ONLY');


function exportRowScores(rows, models) {
  const scored = rows
    .filter((row) => row.pos === 'QB' || row.pos === 'WR')
    .map((row) => {
      const features = makeFeatures(row)
      ...
      const out = {
        ...
        hasSeasonPff: row.seasonPffScore != null,
        globalAv: predictAv(models.GLOBAL_WITH_QB_WR_SEASON_CONTEXT, features),
        qbAv: null,
        wrAv: null,
      }

      if (row.pos === 'QB') {
        out.qbAv = predictAv(models.QB_ONLY, features)
      }

      if (row.pos === 'WR') {
        out.wrAv = predictAv(models.WR_ONLY, features)
      }
  ...
  fs.writeFileSync(
    'public/data/model/v5_qb_wr_row_scores.json',
...
const report = {
  ...
  modelVersion: 'V5-QB-WR-test',
  meta,
  samples: {
    all: rows.length,
    qb: qbRows.length,
    wr: wrRows.length,
    qbWr: qbWrRows.length,
  },
  globalAll: globalAll.report,
  qbWr: qbWr.report,
  qbOnly: qbOnly.report,
  wrOnly: wrOnly.report,
  walkForward: {
    qb: walkForward(qbRows, 'QB'),
    wr: walkForward(wrRows, 'WR'),
    qbWr: walkForward(qbWrRows, 'QB_WR'),
  },
  models: {
    GLOBAL_WITH_QB_WR_SEASON_CONTEXT: globalAll.model,
    QB_WR_COMBINED: qbWr.model,
    QB_ONLY: qbOnly.model,
    WR_ONLY: wrOnly.model,
  },
};

const rowScores = exportRowScores(rows, {
  GLOBAL_WITH_QB_WR_SEASON_CONTEXT: globalAll.model,
  QB_ONLY: qbOnly.model,
  WR_ONLY: wrOnly.model,
});
...
fs.writeFileSync('public/data/model/v5_qb_wr_test_report.json', JSON.stringify(report, null, 2));
fs.writeFileSync('public/data/model/v5_qb_wr_models.json', JSON.stringify(report.models, null, 2));

console.log(JSON.stringify({
  modelVersion: report.modelVersion,
  meta: report.meta,
  samples: report.samples,
  globalAll: report.globalAll,
  qbWr: report.qbWr,
  qbOnly: report.qbOnly,
  wrOnly: report.wrOnly,
  walkForward: report.walkForward,
  rowScores: rowScores.length,
}, null, 2));
