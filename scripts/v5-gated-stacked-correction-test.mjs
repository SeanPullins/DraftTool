...
function defaultGatePolicies() {
  return [
    {
      name: 'gate_all_clamp_35',
      maxCorrection: 35,
      requireData: false,
      groups: ['QB', 'SKILL', 'OL', 'FRONT', 'DB'],
    },
    {
      name: 'gate_all_clamp_15',
      maxCorrection: 15,
      requireData: false,
      groups: ['QB', 'SKILL', 'OL', 'FRONT', 'DB'],
    },
    {
      name: 'gate_all_clamp_10',
      maxCorrection: 10,
      requireData: false,
      groups: ['QB', 'SKILL', 'OL', 'FRONT', 'DB'],
    },
    {
      name: 'gate_data_only_clamp_15',
      maxCorrection: 15,
      requireData: true,
      groups: ['QB', 'SKILL', 'OL', 'FRONT', 'DB'],
    },
    {
      name: 'gate_qb_skill_db_clamp_15',
      maxCorrection: 15,
      requireData: false,
      groups: ['QB', 'SKILL', 'DB'],
    },
    {
      name: 'gate_non_front_clamp_15',
      maxCorrection: 15,
      requireData: false,
      groups: ['QB', 'SKILL', 'OL', 'DB'],
    },
    {
      name: 'gate_qb_only_clamp_15',
      maxCorrection: 15,
      requireData: false,
      groups: ['QB'],
    },
    {
      name: 'gate_skill_only_clamp_15',
      maxCorrection: 15,
      requireData: false,
      groups: ['SKILL'],
    },
...
function passesGate(row, policy) {
  if (!policy.groups.includes(row.group)) return false;

  if (policy.requireData && !row.pff && row.seasonPffScore == null) {
    return false;
  }
...
function applyGatedStackModel(rows, model, policy) {
  return rows.map((row) => {
    const stackFeatures = makeStackFeatures(row);
    const rawCorrection = predictCorrection(model, stackFeatures);

    const correction = passesGate(row, policy)
      ? clamp(rawCorrection, -policy.maxCorrection, policy.maxCorrection)
      : 0;

    const gatedScore = clamp((row.v4Percentile ?? 50) + correction, 0, 100);

    return {
      ...row,
      rawCorrection,
      correction,
      gatePolicy: policy.name,
      predRankScore: gatedScore,
    };
  });
}

function summarizeGateReports(results) {
  const policies = defaultGatePolicies();
  const output = {};

  for (const policy of policies) {
    const reports = results.map((row) => row.gated[policy.name]);

    output[policy.name] = {
      avgPairwiseAccuracy: mean(reports.map((r) => r.pairwise.accuracy)),
      avgWeightedPairwiseAccuracy: mean(reports.map((r) => r.pairwise.weightedAccuracy)),
      avgSpearman: mean(reports.map((r) => r.spearman)),
      avgTop50ActualOutcome: mean(reports.map((r) => r.top50.avgActualOutcome)),
      avgTop50WeightedAv: mean(reports.map((r) => r.top50.avgWeightedAv)),
      avgTop50Busts: mean(reports.map((r) => r.top50.busts)),
      avgTop100Busts: mean(reports.map((r) => r.top100.busts)),
    };
  }
...
function chooseBestGate(gateAverages) {
  return Object.entries(gateAverages)
    .map(([name, report]) => ({ name, ...report }))
    .sort((a, b) => {
      const weighted = b.avgWeightedPairwiseAccuracy - a.avgWeightedPairwiseAccuracy;
      if (Math.abs(weighted) > 0.000001) return weighted;

      const pairwise = b.avgPairwiseAccuracy - a.avgPairwiseAccuracy;
      if (Math.abs(pairwise) > 0.000001) return pairwise;

      return b.avgSpearman - a.avgSpearman;
    })[0];
}
...
function holdoutTest(rows, trainStart, trainEnd, testStart, testEnd, label) {
  ...
  const gated = Object.fromEntries(
    defaultGatePolicies().map((policy) => [
      policy.name,
      evaluate(applyGatedStackModel(testRows, correctionModel, policy)),
    ])
  );
  ...
  return {
    ...
    gated,
    bestGate: chooseBestGate(Object.fromEntries(
      Object.entries(gated).map(([name, report]) => [
        name,
        {
          avgPairwiseAccuracy: report.pairwise.accuracy,
          avgWeightedPairwiseAccuracy: report.pairwise.weightedAccuracy,
          avgSpearman: report.spearman,
          avgTop50ActualOutcome: report.top50.avgActualOutcome,
          avgTop50WeightedAv: report.top50.avgWeightedAv,
          avgTop50Busts: report.top50.busts,
          avgTop100Busts: report.top100.busts,
        },
      ])
    )),
    v4: evaluate(v4Only),
...
function rollingModern(rows) {
  ...
  for (const testYear of years) {
    ...
    results.push({
      ...
      gated: result.gated,
      bestGate: result.bestGate,
      v4: result.v4,
    });
  }

  const gateAverages = summarizeGateReports(results);
  ...
  return {
    ...
    gateAverages,
    bestGateByWeightedPairwise: chooseBestGate(gateAverages),
  };
}
...
const report = {
  ...
  modelVersion: 'V5.7-gated-stacked-correction-test',
  status: 'experimental',
  meta,
  samples: {
  ...
  holdout_2018_2021: {
    ...
    gated: holdout_2018_2021.gated,
    bestGate: holdout_2018_2021.bestGate,
    v4: holdout_2018_2021.v4,
  },
  modernHoldout_2018_2021: {
    stacked: modernHoldout_2018_2021.stacked,
    gated: modernHoldout_2018_2021.gated,
    bestGate: modernHoldout_2018_2021.bestGate,
...
fs.writeFileSync('public/data/model/v5_gated_stacked_correction_report.json', JSON.stringify(report, null, 2));
fs.writeFileSync('public/data/model/v5_gated_stacked_correction_models.json', JSON.stringify(report.models, null, 2));
